...
PS> Get-MileageByState -Credential Get-Credential -StartingDate '10/31/2016' -EndingDate '10/31/2016 23:59:59'
...
function Get-MileageByState
{
    ...
    $url = "$endpoint`?action=showposition&operation=mileage&format=xml&fromdate=$startingEpoch&todate=$endingEpoch&loicategory=States"
    Write-Debug $url
    ...
    Write-Verbose "Get-MileageByState: $( (Get-Date) - $start)"
...
Set-Alias mileagebystate Get-MileageByState
